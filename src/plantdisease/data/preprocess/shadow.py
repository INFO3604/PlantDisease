...
def remove_shadows_hsv_threshold(
    image: np.ndarray,
    leaf_mask: np.ndarray | None = None,
) -> np.ndarray:
    """Remove shadows using HSV color-space thresholds.

    A shadow mask is generated by identifying pixels with low Value
    (brightness) and low Saturation in HSV space.  The original image is
    processed using a bitwise AND with the inverse of the shadow mask,
    effectively zeroing out shadow pixels.

    Parameters
    ----------
    image : np.ndarray  (BGR, uint8)
    leaf_mask : np.ndarray | None
        If provided, only shadow pixels within the leaf region are removed.

    Returns
    -------
    np.ndarray  -- shadow-removed BGR image (uint8).
    """
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

    # Shadows: low brightness (V < 80) and low saturation (S < 50)
